...
int main() {
    ...
    int N,player_turn; int choixmenu,direction;int direction_bar,c;
    int plateaujeu[17][17]={{0}};
    const char * sauvegarde="s.txt";
    const char * score="scorebon.txt";;
    joueur joueur1;
    joueur joueur2;
    joueur joueur3;
    joueur joueur4;
    ...
    joueur3.score=0;
    joueur4.score=0;
    Chronometre chronometres[4]; // Un chronomètre par joueur
    int temps_total = 120; // Temps total alloué par joueur en secondes
        ...
        switch(choix) {
            ...
            if(N==2) {
                ...
                do {
                    ...
                    for (int i = 0; i < N; i++) {
                        initialiser_chronometre(&chronometres[i], temps_total);
                    }
                    for(int i=0;i<=1;i++) {
                        do{
                        printf("Tour du joueur %d %s\n",player_turn,joueur_actif.nom);
                        printf("nombres de barrieres restantes %d\n",joueur_actif.nb_barr);
                        ...
                        demarrer_chronometre(&chronometres[player_turn - 1]);
                        switch(choixmenu) {
                        ...
                        if(i==1) {
                            ...
                            int temps_restant = obtenir_temps_restant(&chronometres[player_turn - 1]);
                            printf("Temps restant pour %s : %d secondes\n", joueur_actif.nom, temps_restant);
                        if (temps_ecoule(&chronometres[player_turn - 1])>0) {
                            printf("Temps écoulé pour %s !\n", joueur_actif.nom);
                            continue; // Fin du tour
                        }
                        if (temps_ecoule(&chronometres[player_turn - 1])<0) {
                            printf("%s a dépassé son temps, pénalité de 2 points appliquée.\n", joueur_actif.nom);
                            joueur_actif.score -= 2;
                        }
                            if(d==0) {
                                break;
                            }
                ...
                //score
                sauvegardescore(tab_joueur,N,score);
                mettre_a_jour_score(tab_joueur,N,score);
            }else {//4joueurs
                printf("Joueur 1 entre son nom\n");
                scanf("%s",&joueur1.nom);
                printf("Joueur 2 entre son nom\n");
                ...
                do {
                    ...
                    for(int i=0;i<=1;i++){
                        do {
                            ...
                            switch(choixmenu) {
                                ...
                                break;
                                case 2://barrieres
                                    printf("Nombre de barrieres restantes: %d\n",joueur_actif.nb_barr);
                                printf("Entrer les coordonnees de la demi-barriere\n");
                                scanf("%d %d",&x,&y);
                                printf("Entrer la direction\n");
                                printf("1. Haut\n");
                    ...
                    tab_joueur[(player_turn-1)%3]=joueur_actif;
                    tab_joueur[player_turn%3]=adversaire1;
                    tab_joueur[(player_turn+1)%3]=adversaire2;
                    tab_joueur[(player_turn+2)%3]=adversaire3;
